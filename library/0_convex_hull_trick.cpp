...
// NOTE: 最大値の場合、コード中に書き換えが必要な箇所が二箇所あることに注意！！！
template <typename T>
...
struct CHT2 {
    ...
    void add(ll a, ll b) {
        ...
        C.erase(cp(*it_m1(pos), *it_p1(pos)));
        {
            // 右方向の削除
            It it = it_m1(pos);
            while (it != S.begin() && check(*it_m1(it), *it, p))
                --it;
        ...
        }
        {
            // 左方向の削除
            It it = it_p1(pos);
            while (it_p1(it) != S.end() && check(p, *it, *it_p1(it)))
                ++it;
            C_erase(++pos, it);
            S.erase(pos, it);
            pos = S.find(p);
  ...
  private:
    template <class T>
    T it_p1(T a) { return ++a; }
    template <class T>
    T it_m1(T a) { return --a; }
    struct L {
        ll a, b;
        L(ll a, ll b) : a(a), b(b) {}
